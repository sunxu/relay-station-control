...
当前增强后的 `postgres` gate 在隔离 PostgreSQL 18 上覆盖 Migration 不回填、首次基线、连续缺失、恢复、out-of-scope/re-add、权限/保护写、policy-mutation shell 开关，以及 1/10/50 Node × 1000账号的 lifecycle 行数、WAL、锁等待和事务/批次预算。只有增强 gate 在最终提交上实际通过后，才能保留其新成功分类；它仍不声称提交未知或完整 canary 路径已覆盖。`container` 先运行该数据库 gate，再经 pinned official-image synthetic gate执行唯一一次 GET，并在同一次生产 Driver/Worker/Store 链路中断言一个 `present` lifecycle 行。`data-plane` 在已形成两行 lifecycle 后停 PostgreSQL，停机窗口运行独立 synthetic loopback `50/50`，重启后再次通过 Store 和持久指标读取两行；它不访问 Gateway 或 Node。

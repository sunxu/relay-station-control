...
Driver 本身不负责 poll-run 调度，也不保存账号状态；独立的 Account Inventory runtime 负责按 durable poll-run/lease 调用 Driver。详见[Account Inventory poll-run 运行手册](account-inventory-poll-runs.md)与[Gateway Directory runtime 运行手册](gateway-directory-runtime.md)。Driver 只执行一次管理只读请求，不创建任务、不修改资产、Node、Gateway 或 CLIProxyAPI 状态。

Driver 只表示一次内存观察。`/healthz` 成功不代表账号池完整、Provider 可用、Gateway enabled 或 Node 可调度；runtime 的成功与失败由其 poll-run 证据记录。

生产启动使用显式开关 `CONTROL_CLIPROXYAPI_DRIVER_ENABLED`，默认为 `false`。关闭时 registry 为空且不能根据数据库字符串动态加载 Driver；开启时只登记 `cliproxyapi.auth-files.v1` 和两项只读 capability。Driver 仍不自行调度；启用的 Account Inventory runtime 才会按其运行开关和 durable poll-run 调用对应 capability。
...
Driver registry 启动只校验登记、Secret mapping 和运行时预算；每次 Driver 调用仍校验 endpoint 结构、capability、Secret 内容和响应契约。日志不包含环境变量原值。已退役的 DNS/CIDR/CA 环境变量会被忽略，即使残留、非法或指向不存在的文件，也不恢复旧策略或阻止启动。

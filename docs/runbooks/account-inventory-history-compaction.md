...
- GitHub Actions先运行`history_static`，再将PostgreSQL、process、rollback和data-plane四个Docker-heavy stage分配到独立runner；最终`postgres_history`只依赖所有stage成功并输出固定`mode=all` marker，不交换或解析原始artifact。本地`account-inventory-history-run.sh all`继续保留同等门禁语义；各mode只额外输出固定stage名和整数毫秒耗时；

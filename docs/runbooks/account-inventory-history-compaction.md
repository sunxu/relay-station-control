...
本手册描述阶段 3 `add-control-account-inventory-history-compaction` 的运行契约。**最终候选已完成 OpenSpec、故障、容量、隐私、旧二进制、全仓 race 和发布工程门禁，并具备 Migration 9、确定性核心算法、严格 compatibility/Store adapter、受控 compaction/daily-rollup/retention 状态机、低基数 metrics，以及接入 `cmd/control/main.go` 的默认关闭运行循环。** 工程验收完成不等于已获生产启用批准；在运营责任人显式批准并按本手册执行分阶段 rollout 前，必须保持默认关闭，也不得声称已投产。
...
真实fenced-finalize矩阵进一步以A槽current、C槽较新health和中间B槽证明stale判定使用已持久health watermark：A<B<C时B只保留`stale_poll`证据，不倒退health或推进snapshot/lifecycle。并发finalize与策略scope切换只允许完整串行结果，scope先行不刷新health；从未promotion的失败结果不创建Provider current、snapshot或lifecycle。该两路矩阵本身不作为四路并发证据。

Poll cleaner在稳定锁定目标poll集合后，按`instance_id,provider`显式预锁引用这些poll的Provider current rows，再按`instance_id,provider,account_key`预锁account current rows；这与fenced promotion和scope切换共同使用Provider→account顺序。PostgreSQL18同链fixture以`retention_first`和`scope_first`两种顺序并发运行到期current-source retention、真实Repository current query、更新槽promotion和即时Provider scope切换；`pg_stat_activity`确认另外两条writer等待，未提交期间Repository在2秒内只看到基线A present/Z suspected-missing，先行事务提交后全员在有界context内成功且无timeout/deadlock。两种顺序都得到retention `processed=2/deleted=4`、poll/Provider-result进度`2/2`、旧poll归零、新poll/result各一、scope audit一条及两个完整out-of-scope账号；retention先行时promotion applied、Provider/A/Z均指向新poll且health为新槽，scope先行时result为`policy_changed`、三类pointer均为NULL且health保留旧poll2槽。这完成6.9，但不关闭9.4的故障注入或9.6的全仓Go race验收，也不形成性能SLO。
...
数据库renew/reconcile和运行循环已实现并有定向测试。Compaction、rollup worker和单个有序retention scanner共同使用`CONTROL_ACCOUNT_INVENTORY_HISTORY_CONCURRENCY`这一总上限，不各自获得一份额度。Retention始终按poll/children→segment/final→completed rollup run（同事务持久化retired-day cutoff）→completed compaction run调用四个有界事务；某次提交结果未知时不在内存立即重放，而是等待下一次扫描由数据库持久真相重新选择候选。当前循环把state/day/count/checksum/activation/repository不一致视为全局fatal：先停止planner和所有其他新claim/新事务；触发异常的原Worker只允许执行一次带原fencing的terminal-fail事务来持久化固定失败原因，其他Worker仅排空已经进入的单个有界事务，然后history service进入固定`runtime_stopped`。它不会自动越过异常键继续删除或发布rollup。真实Control process验收以`CONTROL_DATABASE_MAX_CONNS=1`和owner表锁证明唯一连接在生产summarize中等待，另一个metrics请求有界等待pool且Control保持存活；SIGTERM的grace内解锁分支只提交当前summary、未到期lease在重启后不被接管并最终由Reconciler以attempt 2完成，持续持锁分支则由statement timeout原子回滚并只写固定failed记录。既有pending/summarized/deleting三阶段Control/PostgreSQL重启矩阵仍逐项证明Reconciler独占接管。source-backed扩展还精确观察同进程两个worker同时等待、旧fence在新claim后零影响、PostgreSQL outage/pool exhaustion后完成，以及statement timeout保留2条source再由retry claim完成；权限撤销和`23514` trigger按生产语义固定terminal internal/runtime stopped且source保留，不声称自动恢复。实际PID停止均有界；这完成7.4和9.4。

生产只运行一个planner以避免重复调度；既有并发planner数据库测试证明scheduler竞争仍幂等，同一Control的真实并发证据锁定在两个worker。Planner、rollup finalize或poll retention的Repository fatal都固定进入`runtime_stopped`，但只有compaction原Worker按既有语义写terminal fail；rollup保留pending owner/fence/lease与source proof/segments且零failure audit，planner/retention保留source且零history success/failure audit。不要在运维或测试中伪造不存在的rollup/retention terminal fail。
...
5. 保留已通过的 PostgreSQL 18 `1/10/50 Node、每档总计1,000账号`短slot功能矩阵、完整敏感canary、旧二进制forward-schema、86.4万/115.2万snapshot容量、全仓race、发布门禁和独立数据面隔离证据。
...
对应dry-run固定使用两个真实窗口：process窗口先以默认disabled启动候选并通过compatibility，再用`concurrency=1`、snapshot delete batch `1`处理独立2-snapshot合成UTC日，验证source/deleted守恒、唯一completed final、真实promoted current account的完整current-query行在cleanup前后逐字段一致（覆盖query返回的全部17个产品字段，含全部nullable时间戳，并经生产AccountInventoryRepository DTO投影路径校验；空集合对空集合不作为等价证据）和外部请求零，之后才进入既有`concurrency=2`双worker阶段；rollback窗口先以disabled启动当前候选，确认兼容且history/history-audit与generic-job指纹不变，有界停止后才启动pinned旧Control。rollback runner不调用Migration down，`production_down=not_used`只在forward schema保留且旧Control验证成功后报告。两个完整窗口已在真实PostgreSQL18通过（process输出`staging_concurrency_1=covered staging_delete_batch_1=covered staging_current_query_equivalent=covered dual_workers_observed=2 external_requests=0`，rollback输出`current_candidate_disabled=covered runner_stopped_before_old_binary=covered history_and_history_audit_unchanged=covered production_down=not_used`，五类残留均为零），10.2据此完成。
...
两个允许的正式规模均在隔离PostgreSQL 18执行完成且`exit 0`，每档运行5个独立summary/rollup样本并远超20个delete batch。以下是完整脱敏聚合值；nearest-rank时延和所有资源数仅描述本次候选执行，不是生产SLO、告警阈值或未来容量承诺。

**provenance 限制（已解决）**：批次37的两档运行发生在最终候选之前，未记录 candidate SHA 且 `gh run list --workflow history-capacity.yml` 无运行记录，故原证据仅作为历史聚合观测、不绑定任何候选。两档已于最终候选 `86e369cb11d3b2268ed9983fff961cc731098389` 本地重跑并 `exit 0`：`864000` 档 UTC `2026-09-01T20:07:17Z`→`20:13:00Z`，`1152000` 档 UTC `2026-09-02T01:13:16Z`→`01:21:03Z`，marker 均含 `evidence=full` 与五类零残留；两档均无OOM/timeout/deadlock，守恒通过。新的候选绑定值见 evidence 文档，原批次37数值被本次重跑取代。

- `864000` snapshots/sample，3 Node，5 samples，870 delete batches：summary P50/P95/P99=`47515/55590/55590 ms`，rollup P50/P95/P99=`143/148/148 ms`；database bytes/growth=`2415621823/2404081664`，index bytes/growth=`1509728256/1508737024`，WAL=`6569937720`；harness max RSS before/after=`20185088/24363008`，PostgreSQL cgroup peak=`3603832832`；blocks read/hit=`6190650/166804575`，temp bytes=`2586302976`，max lock waiters=`0`，deadlock delta=`0`；source/deleted=`4320000/4320000`，account/Provider rollup rows=`15000/15`，conservation=`passed`。无OOM、timeout或deadlock，container/volume/network/temp/lock残留均为零。该次旧success marker只打印前三类Docker残留；strict cleanup与随后独立检查确认temp/lock同为零。
- `1152000` snapshots/sample，4 Node，5 samples，1160 delete batches：summary P50/P95/P99=`61264/74343/74343 ms`，rollup P50/P95/P99=`164/176/176 ms`；database bytes/growth=`3207583423/3196035072`，index bytes/growth=`2003296256/2002305024`，WAL=`8668940096`；harness max RSS before/after=`19972096/26427392`，PostgreSQL cgroup peak=`3892613120`；blocks read/hit=`6067993/219456540`，temp bytes=`3336427520`，max lock waiters=`0`，deadlock delta=`0`；source/deleted=`5760000/5760000`，account/Provider rollup rows=`20000/20`，conservation=`passed`。无OOM、timeout或deadlock，success marker确认container/volume/network/temp/lock残留均为零。
...
- 正式容量门禁已分别以86.4万和115.2万snapshot/sample、各5个summary/rollup样本及870/1160个delete batch通过；完整时延、DB/index/WAL、harness RSS、PostgreSQL peak、buffer/temp/lock/deadlock、守恒及零残留值见上节。这些观察值不构成生产SLO阈值；
- static定向race另锁定`TestRepositoryLoopsUsesConfiguredConcurrency`和`TestRepositoryLoopsFatalMismatchStopsPlannerAndNewWorkerTransactions`；PostgreSQL18 runner以`-race`单独执行retention/current-query/promotion/scope同链并发测试，最终候选的全仓`go test -race ./... -count=1`进一步关闭9.6；
- 敏感canary双门禁已覆盖7类本地success/zero/partial/permission/timeout/restart/cleanup-failure sink和真实PostgreSQL18 partial/zero/permission/timeout/reconnect数据库非身份列扫描，aggregate仅在两者成功后声明covered；
- 真实Control process矩阵复用既有fake Node原子计数器作为seeded Node endpoint及Control-only HTTP/HTTPS catch-all proxy。除了默认disabled、zero-source、metrics权限失败、restart/reconcile和shutdown，它还在同一计数窗口运行2-snapshot source-backed retention、`CONCURRENCY=2`双worker、旧fence、`MAX_CONNS=1`、PostgreSQL stop/start、statement timeout恢复、summarize权限撤销、`23514` trigger及planner/rollup-finalize/poll-retention权限fatal；最终固定得到`total/health/inventory/unauthorized/rejected=0`和`external_requests=0 node=0 gateway=0 prometheus=0 internet=0 model=0`，完成8.6/9.4；
...
- 独立data-plane runner复用pinned官方CLIProxyAPI与Bearer认证`/v1/models`：Control/PostgreSQL在线时baseline为`1/1`，两者均已停止且分别确认health/`pg_isready`不可用后仍为`100/100`；脱敏结果固定`scope=data_plane_isolation gateway_inference_e2e=not_covered`且container/volume/network/temp/lock残留全零。这是进程隔离证据，不是Gateway inference E2E；
- 七个history acceptance runner的成功报告已统一为固定container/volume/network/temp/lock零残留字段；契约逐个拒绝identity形状字段并验证全部invalid-arg固定分类。启动隔离PostgreSQL与pinned数据面后注入migration失败的门禁仅输出`migration_failed`，随后复核五类资源残留均为零；
- GitHub Actions将`history_static`、PostgreSQL、process、rollback和data-plane分配到五个独立runner并发执行；static不产生heavy stage依赖的状态或artifact。最终`postgres_history`只依赖所有stage成功并输出固定`mode=all` marker，不交换或解析原始artifact。本地`account-inventory-history-run.sh all`继续保留同等门禁语义；各mode只额外输出固定phase/stage名和整数耗时；
- 因此 `CONTROL_ACCOUNT_INVENTORY_HISTORY_ENABLED=true` 已通过工程验收，但默认值继续为`false`；只有运营责任人显式批准并按本手册完成兼容性确认、staging单Worker和逐步扩容后才可在生产启用。

最终候选已执行：两次可复现生成、全部Go/test/race/vet/build、前端typecheck/test/build、OpenSpec strict、Migration 9 PostgreSQL 18 up/down/up与ACL、history状态/故障/容量/retention、旧二进制forward-schema、敏感canary、外部请求零和data-plane isolation。所有Go/npm/Docker/make命令显式清除大小写HTTP/HTTPS/ALL proxy；PostgreSQL使用隔离测试资源且不把连接串、SQL参数或原始日志保留到artifact。

本手册记录的是已完成工程验收的运行契约，不是生产启用或投产记录。生产状态只能由实际 rollout、监控观察和运营批准记录证明。

...
**provenance 限制**：这两次容量运行发生在最终候选之前，未记录 candidate commit SHA，`gh run list --workflow history-capacity.yml` 亦无任何 GitHub Actions 运行记录，因此无法绑定 workflow run ID，也不允许猜测或伪造。该证据仅作为历史聚合观测；若发布策略要求正式容量证据与最终 candidate SHA 绑定，`864000` 与 `1152000` 两档必须在最终候选上通过`deploy/acceptance/account-inventory-history-capacity.sh`重跑。

...
- 真实Control process矩阵现复用既有fake Node原子计数器作为seeded Node endpoint及Control-only HTTP/HTTPS catch-all proxy；默认disabled、zero-source主链、metrics权限失败、持锁timeout/pool exhaustion、restart/reconcile和shutdown期间固定得到`total/health/inventory/unauthorized/rejected=0`。该门禁仍标记`external_requests=partial_process_paths`：retention只是空扫，permission/cleanup与其余错误路径尚未全部置于同一counter，8.6必须等待9.4 source-backed fault matrix扩展；

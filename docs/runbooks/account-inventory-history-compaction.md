...
- 纯 compaction模型与循环覆盖 `pending → summarized → deleting → completed`、`failed_from`恢复、stale fencing零影响、unknown-commit持久状态决策和删除守恒；daily-rollup模型与循环覆盖可恢复失败allowlist、renew/reclaim、completed不可变、unknown-finalize有界重放和固定错误分类；

...
- An initial complete PostgreSQL-backed `internal/store` run against the long-lived development database reproduced four asset-registry failures: URL percent-escape case, NULL gateway SQLSTATE, provider-policy owner mutability and scheduled-disable replay timestamp equality. An isolated archive of the then-current Git `HEAD`, which contained neither migration `00004` nor durable-job code, reproduced the same assertions against that database, excluding this change as their cause.
- The follow-up clean-room run created a disposable PostgreSQL 18 database, applied migrations `00001` through `00004`, inserted the required environment singleton and ran `go test -race -count=1 ./internal/store/...`; the complete suite passed with exit code `0`. This proves the four earlier results came from schema drift in the long-lived development database rather than a source or forward-Migration defect, so no corrective `00005` or asset-registry behavior change is required.

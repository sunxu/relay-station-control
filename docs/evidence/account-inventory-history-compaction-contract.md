...
This document is the review crosswalk for OpenSpec tasks 1.1, 1.5, 1.6, 2.11, and 8.3. It records the approved contract and the bounded implementation evidence listed below. It is not evidence that the complete production runtime, capacity, canary, fault-injection, or release gates have passed. Those claims require their task-specific executed tests on the final candidate.
...
This evidence closes 2.11 and 8.3 only. It does not close 1.5 or 8.4: unique canaries still have to be traced through every database non-identity column, log, metric, error, and artifact across success and all required failure/restart/cleanup paths.
...
This crosswalk supports completion of 1.1, 1.6, 2.11, and 8.3 only with their cited executed gates. It does not complete 1.5 or 8.4: those remain open until the full cross-sink canary matrix passes. It must not be used to mark unrelated schema/ACL, UI/OpenAPI, capacity, fault-injection, or release tasks complete without their separately executed evidence.

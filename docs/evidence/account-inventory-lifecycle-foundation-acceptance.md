...
| Capacity | 1/10/50 Nodes × 1000 accounts acquire concurrency before lease, then measure lifecycle rows, WAL, lock waiters, last dispatch and transaction/batch duration; last dispatch remains inside 120-second grace and each finalize inside its 30-second lease | direct database fixture; 0 network | passed: isolated capacity gate |

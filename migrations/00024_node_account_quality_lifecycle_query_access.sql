...
CREATE FUNCTION public.control_query_node_account_quality_v2(
    target_instance_id uuid, target_provider text, target_lifecycle text, target_quality text,
...
BEGIN
    IF target_instance_id IS NULL OR target_provider IS NULL OR target_quality IS NULL
       OR target_lifecycle IS NULL OR target_lifecycle NOT IN ('','present','suspected_missing','missing','out_of_scope')
    ...
    LOOP
        FOR inv IN SELECT * FROM public.control_query_current_account_inventory_v1(
            target_instance_id, target_provider, target_lifecycle, '', '', cursor_key, 101
        ) LOOP
            scanned := scanned + 1;
...
ALTER FUNCTION public.control_query_node_account_quality_v2(uuid,text,text,text,text,interval,integer) OWNER TO relay_control_migrator;
REVOKE ALL ON FUNCTION public.control_query_node_account_quality_v2(uuid,text,text,text,text,interval,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.control_query_node_account_quality_v2(uuid,text,text,text,text,interval,integer) TO relay_control_runtime;

-- +goose Down
DROP FUNCTION public.control_query_node_account_quality_v2(uuid,text,text,text,text,interval,integer);

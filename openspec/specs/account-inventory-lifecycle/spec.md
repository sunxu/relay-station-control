...
Control MAY 提供仅供内部 Store 使用的有界、稳定排序 lifecycle 读取函数和按受控 instance/provider/lifecycle/固定 transition reason 聚合的指标。标准化 email/account key MAY 只进入受保护 snapshot、duplicate 和 lifecycle 列以及授权内部返回值；它们与 poll/policy ID、版本/提交、endpoint/IP、Secret、header/body、原始错误 MUST NOT 进入普通日志、Prometheus 标签、错误文本、SQL 参数日志、测试输出或验收 artifact。运行时角色 MUST 无任意 lifecycle DML 权限。

#### Scenario: 内部读取生命周期
- **WHEN** 后续内部消费者按 instance/provider 与有界 limit 读取当前 lifecycle
- **THEN** 数据库以稳定 key 顺序返回白名单字段，不开放产品 HTTP 路由或无界全表枚举

#### Scenario: 敏感 canary 贯穿转换与失败路径
- **WHEN** 验收向 email/account key、Secret、endpoint和原始错误注入唯一 canary
- **THEN** 只有受保护身份列和授权内部读取可包含标准化身份，日志、指标、错误和 artifact 不包含 canary

#### Scenario: lifecycle 指标导出
- **WHEN** Control 导出当前状态或转换结果
- **THEN** 标签只包含受控 instance/provider/lifecycle/fixed reason且不包含逐账号身份或高基数字段

### Requirement: lifecycle foundation MUST 保持产品与数据面边界

本 change MUST NOT 新增或修改产品 OpenAPI、生成客户端、React 页面、账号导出、人工状态操作、补采或 promotion 入口；MUST NOT 删除账号、实现保留清理、HMAC account ID、跨 Node 重复、趋势/覆盖率/摘要/压缩或告警路由。它 MUST NOT 修改 Gateway/Node，也不得增加现有固定账号清单只读 GET 之外的网络请求。Control/PostgreSQL/lifecycle 停止只暂停状态推进，不得影响既有模型流量。

#### Scenario: 管理员访问现有产品界面
- **WHEN** lifecycle foundation 部署后管理员访问现有 API/UI
- **THEN** 现有契约保持兼容且没有新增账号页面、导出、状态修改或告警入口

#### Scenario: 网络调用范围验收
- **WHEN** lifecycle、恢复和策略切换验收运行
- **THEN** Node 请求数量与既有 poll-run 一致，不调用 Gateway、模型数据面、Node 写接口或未登记目标

#### Scenario: Control 或 PostgreSQL 停止
- **WHEN** lifecycle 处理依赖停止或旧应用回滚
- **THEN** 已形成状态保留、新状态暂停推进，Gateway 与 Relay Node 已有模型请求继续

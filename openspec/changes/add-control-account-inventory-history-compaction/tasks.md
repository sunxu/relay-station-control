...
- [x] 10.5 运行Migration9 schema/ACL/rollback、旧二进制forward-schema、PostgreSQL18功能/故障/容量、零外部请求、敏感canary和数据面隔离完整验收，保存脱敏证据摘要
...
> 第四十二批执行`account-inventory-history-run.sh all`完整验收：static exact discovery/race/定向race/百万行、safety 7类本地sink、Migration8→9指纹、PostgreSQL18全功能/故障/ACL/1-10-50容量、process零外部请求全窗口、rollback pinned旧二进制forward-schema、data-plane isolation baseline 1/1与outage 100/100全部通过；最终固定报告`account_inventory_history_acceptance=success mode=all exact_discovery=covered race=covered history_targeted_race=covered million_rows=covered process=covered rollback_gate=covered sensitive_canary=covered local_sink_canary=covered sensitive_canary_database_sinks=covered fake_network_counter=covered external_requests=0 data_plane_isolation=covered baseline_models=1/1 outage_models=100/100 gateway_inference_e2e=not_covered cleanup_containers=0 cleanup_volumes=0 cleanup_networks=0 cleanup_temp=0 cleanup_lock=0`并已存档脱敏证据摘要，据此完成10.5。

> 第四十三批修复staging current-query等价断言的空集合盲区：source-backed seed fixture现在通过受控写路径（`relay_control.lifecycle_write=finalize` gate + guard自动从applied result推导Provider health）创建一个真实promoted current account及Provider state；seed测试在history运行前断言current query恰好返回1行且account_key/provider/lifecycle/basic_status/source时间/health/freshness逐字段等于预期，convergence在compaction/rollup/retention后断言同一行指纹逐字段不变（M9 query从state投影，poll retention置空FK不改变输出），zero-source窗口仍要求0行。staging_current_query_equivalent从此证明非空current inventory在history cleanup前后完整保留，不再以空集合对空集合充当等价证据。

> 第四十四批将`all`模式的四个PostgreSQL/Docker阶段（postgres/process/rollback/data-plane）改为并行执行：static门禁先行，四阶段后台启动后`wait`聚合退出码，任一失败整体`parallel_stage_failed`，stdout/stderr按阶段落盘后仅grep回放固定success/failed marker（遵守shell contract禁止`cat "$runtime_directory"`原始日志的约定），最后仍输出同一`mode=all`汇总marker。真实运行墙钟从约348s降至237s（约1.5×，上限为process阶段约180s），各阶段marker与五类清理残留零不变。

> 第四十五批把staging current-query等价升级为完整产品行验证：`currentInventoryRow`补齐生产`control_query_current_account_inventory_v1`返回的全部17个字段（新增`first_seen_at`/`last_seen_at`及nullable `missing_since`/`out_of_scope_since`/`last_refresh_at`/`next_retry_at`，`source_updated_at`按nullable指针比较），before/after逐字段相等；另在每个窗口收敛后一次性复用生产`AccountInventoryRepository`走真实sqlc JSON编码、strict decode、validation与DTO投影路径（该调用会写产品view audit，故只在收敛后执行一次而非轮询内）。`verify_enabled_convergence`相应补传`CONTROL_HISTORY_PROCESS_RUNTIME_DATABASE_URL`。zero-source窗口仍要求0行；snapshot删除完成、source/deleted守恒、compaction/rollup唯一completed与外部请求零均由既有convergence与fake-network counter断言。验证：seed测试2/0两场景、完整process runner全窗口、全仓`go test -race ./...`、shell contract、`make test`/`make build`/`go vet`/actionlint、两次`make generate`零差异均通过。
>
> 同批在最终代码candidate `441e267`（执行树与e8c9834+本次代码改动逐字节一致，其上的纯文档提交不改动任何执行产物）重跑`account-inventory-history-run.sh all`（UTC `2026-09-01T19:40:48Z`→`19:44:56Z`，exit 0），最终固定marker与各阶段零残留一致，provenance（candidate SHA/UTC窗口/runner/exit code/fixed marker）已写入evidence文档；本地执行，无GitHub Actions run ID（CI `postgres_history` job会在push时验证，parent `484b68b`的CI history job已通过）。9.5两档大容量证据（864000/1152000）无candidate SHA且`gh run list --workflow history-capacity.yml`为空，明确标记provenance受限；若发布策略要求容量证据绑定最终candidate SHA，两档均需在最终候选上重跑，禁止伪造run ID或SHA。

> 第四十六批闭环9.5容量provenance：最终candidate `86e369c` 的CI run `33552267164` 全job通过（`PostgreSQL 18 history acceptance` job `100004323471` 13m32s success，最终mode=all marker与本地一致）。两档正式容量在最终候选本地重跑并`exit 0`：`864000`档（3 Node、870 batches，UTC `2026-09-01T20:07:17Z`→`20:13:00Z`）summary P50/P95/P99=`46218/55529/55529ms`、rollup=`118/127/127ms`、WAL=`6535779440`、source/deleted=`4320000/4320000`、conservation=passed；`1152000`档（4 Node、1160 batches，UTC `2026-09-02T01:13:16Z`→`01:21:03Z`）summary=`63607/76473/76473ms`、rollup=`173/179/179ms`、WAL=`8831370256`、source/deleted=`5760000/5760000`、conservation=passed；两档marker均`evidence=full`且五类残留零、无OOM/timeout/deadlock。provenance与聚合值已写入evidence/runbook，取代批次37未绑定候选的旧数值。

> 第四十七批修复并验证CI容量nightly：keep_logs opt-in diagnostic artifact证明CI失败根因是`go test`默认10分钟超时（`FAIL ... 600.011s`）而非磁盘；capacity runner改为`go test -timeout 7h`，并新增脱敏diagnostic artifact（allowlist结果行+canary脱敏+二次扫描，原始日志永不上传）。CI手动重派`864000`档通过（workflow run `33586797984`，job 20m9s success，candidate `a94f8b8`）：summary P50/P95/P99=`157429/161247/161247ms`、rollup=`369/389/389ms`、conservation=passed、五类残留零；CI比本地慢约3.4×。随后手动重派两档完成CI容量门禁：`864000` run `33588761019` 20m54s success（summary=`161087/164971/164971ms`），`1152000` run `33588763693` 26m48s success（summary=`214934/216284/216284ms`、rollup=`463/505/505ms`、conservation=passed、五类残留零），不再依赖nightly cron。

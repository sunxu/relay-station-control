...
- [x] 8.3 `openspec validate add-cross-node-duplicate-ownership --type change --strict --no-interactive` 通过后归档：`openspec archive add-cross-node-duplicate-ownership --yes` 已执行，change 已移动到 `openspec/changes/archive/2026-09-05-add-cross-node-duplicate-ownership`，canonical spec 已生成于 `openspec/specs/cross-node-duplicate-ownership/spec.md`

...
`IMPLEMENTED / VALIDATED / PHASE 8 STAGE 0 CLOSED`。

本文件中的 `Implementation NOT AUTHORIZED`、planning migration 与“本 change 不产生 runtime evidence”等文字记录的是初始 planning 状态，已由 `implementation-validation.md` 的最终状态及 `docs/evidence/phase8-stage0-gate7-acceptance.md` superseded。Stage 0 implementation 已进入当前 main；candidate `c51d149daf3765dc981a445a9c84c26305575592` 是正式验收 candidate，不是当前 main HEAD。

## Why

Phase 8 Stage 0 已把 Relay Node management credential 与 Gateway Directory credential 从运行时文件引用收敛为 Control 持久化的 protected-at-rest state，减少每资产 Secret 文件与引用编排，同时保持 Control 不进入数据面、Gateway/Node 原生职责和全部既有生命周期/命令语义。当前 Requirements、Architecture、Base TCCR 与 Compatibility Addendum 已冻结；本 change 的实现与验证已完成。
...
- Migration：`00051` 已实现并通过 fresh-install、ACL、compatgate 与 runtime evidence；兼容性为 class `4` / floor `4`。

...
```text
FINAL_STAGE0_STATUS = CLOSED
IMPLEMENTATION = VALIDATED
DURABLE_EVIDENCE = PRESENT
FORMAL_CANDIDATE = c51d149daf3765dc981a445a9c84c26305575592
MAIN_INCLUDES_IMPLEMENTATION = YES
```

`FORMAL_CANDIDATE` 是 Stage 0 验收 candidate identity，不是当前 main HEAD。本文更早的 `Push: NOT RUN`、`IN PROGRESS` 或 `NOT STARTED` 文字均为 historical snapshot；当前 authoritative status 由本节和 Gate 7 evidence 定义。

...
- [ ] 3a. 更新 `web/src/auth/AuthContext.tsx` 的 pathname matcher/normalization，使
...
- [ ] 4a. 增加针对 `AuthContext.tsx` pathname matcher 的 unit/integration test，验证
  `/assets` 与 `/assets/` 均解析为 `assets` 路由。
- [ ] 4b. 增加 E2E 场景：authenticated 直接导航 `/assets`、authenticated 直接导航

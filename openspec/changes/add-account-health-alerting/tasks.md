...
- [ ] 3.1c 将字段加入重复 enqueue compatibility、Registry.Catalog/DB catalog comparison、Worker/Reconciler job-policy consistency；执行以持久化 job snapshot 为准，registry/config 不得动态改变旧授权。
...
- [ ] 6.3 在 deploy/acceptance 现有 PostgreSQL/container 体系落实 spec Runtime Acceptance 全部 37 项及 durable-job 增量场景并逐项留证。

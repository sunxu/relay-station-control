...
Control SHALL 以数据库为资产真相源，支持本环境至多一个 Gateway、按稳定 instance ID 唯一的 Relay Node、版本化 Node Driver，以及每个 Node 对固定 capability 的声明。每个 Node MUST 引用已登记的 Driver 合约版本，其 capability MUST 属于该 Driver 版本允许的固定集合；孤立引用、未知 capability 和重复身份 MUST 被数据库拒绝。
...
#### Scenario: 唯一 Gateway
- **WHEN** 受控部署流程尝试为同一环境登记第二个 Gateway
...
Control SHALL 为每个策略作用域保存唯一当前绑定及半开区间 `[effective_from, effective_to)` 的激活历史。当前绑定 MUST 指向同一作用域的现有策略版本；同一作用域的区间 MUST NOT 重叠，生效时间 MUST 使用数据库 UTC 当前时间或未来时间，且不得回填过去时间。

#### Scenario: 读取当前策略
- **WHEN** 某作用域存在当前绑定和覆盖数据库当前时间的激活区间
- **THEN** 只读接口返回同一个当前策略版本及其 active、out-of-scope 集合和生效时间

#### Scenario: 没有当前策略
- **WHEN** 某 Node 作用域没有当前绑定
...
#### Scenario: 绑定跨越作用域
- **WHEN** 当前绑定引用不同环境、Node 类型或 Driver 合约版本的策略
- **THEN** 数据库拒绝该绑定

### Requirement: Node 账号监控状态源自显式激活区间
Control SHALL 为每个 Node 保存半开区间 `[effective_from, effective_to)` 的账号清单监控激活历史，并以数据库 UTC 当前时间是否落在区间内计算当前状态。同一 Node 的区间 MUST NOT 重叠，新区间不得从过去开始；Gateway 状态、Compose 状态或 Node 可达性 MUST NOT 隐式改变监控状态。
...
#### Scenario: 尝试产品写操作
- **WHEN** 客户端向资产路径发送 `POST`、`PUT`、`PATCH` 或 `DELETE`
...
Control SHALL 为已认证管理员提供懒加载的只读资产页面，展示环境、Gateway、Node、Driver/capability、当前 Provider 策略和监控状态。页面 MUST 只调用 Control 的生成客户端，不得直接调用 Gateway、Node 或其他外部服务，也不得显示资产编辑控件或 Secret 引用。

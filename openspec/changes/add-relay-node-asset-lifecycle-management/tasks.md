...
Stage 2 implementation 已获明确授权并通过
`openspec instructions apply add-relay-node-asset-lifecycle-management` 启动。
每项预计不超过 2 小时，按依赖顺序执行并留下可审计 evidence。Task 61 的独立 review 与
closeout evidence reconciliation 已完成；Git/worktree reconciliation 仍需明确授权，archive
readiness 当前为 PENDING GIT CLOSEOUT。

## Shared foundation and compatibility

- [x] 1. 验证并接入 Gateway Stage 1 `asset_admin_command_receipts`、全局 command_id、
  actor-first replay、canonical intent、K1 HMAC、advisory lock、revision helper（≤2h）。
- [x] 2. 为 Node action 注册 shared canonical intent fixtures，覆盖 Register/Edit/Retire/
  Replace 的 field order、patch presence、decimal revision 和 Secret fingerprint（≤2h）。
- [x] 3. 实现/验证 class 2 signed manifest、`relay-control-compat-gate` 和 mandatory
  production wrapper 的 Node evidence path（≤2h）。
- [x] 4. 实现 compatibility floor 1→2 单调更新与 class1 rollback fail-closed acceptance
  （≤2h）。

## PostgreSQL schema and migration

- [x] 5. 在 PG18 隔离库 proof `relay_node_assets` lifecycle columns、backfill active/revision=1
  和 NOT VALID→VALIDATE checks（≤2h）。
- [x] 6. 实现 Node retired shape、revision overflow、retired actor FK RESTRICT 和
  DELETE/identity/lifecycle mutation guard（≤2h）。
- [x] 7. 迁移并冻结 Node mutable/immutable field privileges，禁止 capability ownership
  被 lifecycle path 改写（≤2h）。
- [x] 8. 创建 `relay_node_asset_replacements`、FK/index/UNIQUE/CHECK 和 append-only guard
  （≤2h）。
- [x] 9. 补 lineage fork/merge/cycle/identity reuse 数据库与 transaction tests（≤2h）。
- [x] 10. 增加 monitoring `cancelled_at/cancelled_by/cancel_reason`、FK、reason/shape
  checks 与 privilege boundary（≤2h）。
- [x] 11. 扩展 monitoring history guard：future NULL→non-NULL 一次、禁止 uncancel/修改
  schedule/cancel current-past/DELETE/TRUNCATE（≤2h）。
- [x] 12. 用真实 PostgreSQL 18 验证 generated stored `active_range` expression alteration
  是否允许；记录可执行 proof 和失败 rollback behavior（≤2h）。
- [x] 13. 按“drop dependent GiST exclusion（如需要）→change generated expression→recreate
  exclusion→validate”实现 additive active-range migration proof，确认旧 rows 保留（≤2h）。
- [x] 14. 以 `'empty'::tstzrange` 验证 GiST overlap、current eligibility、expected-slot
  exclusion 和 cancelled row reach-effective-from 行为（≤2h）。
- [x] 15. 扩展 binding end reason CHECK/guard 为 `node_retired|node_replaced`，验证
  immutable history 与 actor FK（≤2h）。
- [x] 16. 更新 compatibility marker/floor migration，证明旧 runtime 在 floor2 下不能启动
  （≤2h）。
- [x] 17. 在 `account_inventory_poll_runs` 增加 `dispatch_authorized_attempt`、
...
- [x] 17a. 扩展既有两层 promotion-skip CHECK allowlist（不新增字段）：
  ...
  provider_duplicate|stale_poll` 基础上 additive 扩展
  `monitoring_ineligible|node_retired|node_replaced`；更新 promotion validator，接受
  finalized evidence-only `monitoring_ineligible`，拒绝该 reason 与
  `promotion_applied=true`，并强制 run 与全部 pinned active Provider reason 一致；验证既有
  `promotion_terminal`/`promotion_shape` guard 与 PG18 migration（≤2h）。
- [x] 17b. 更新 `control_refresh_account_inventory_provider_health_v1()` 触发器函数
  ...
  同时排除 `monitoring_ineligible`/`node_retired`/`node_replaced`，provider 级排除条件从
  `policy_changed`/`stale_poll` 扩展为同时排除
  ...
  保持既有 health refresh 语义不受影响；`monitoring_ineligible` → historical evidence 保留且
  health/current truth 不变；`policy_changed`/`stale_poll` → 既有行为不变（≤2h）。
- [x] 18. 创建 `asset_registry_generations` 单例表（`singleton_id=1`、
...
- [x] 19. 实现 Node Register transaction：new identity absent、capability validation、
  revision=1、audit/receipt atomicity（≤2h）。
- [x] 20. 实现 Node Edit transaction：只允许 display/endpoint/opaque secret patch、
  expected_revision 和 revision overflow（≤2h）。
- [x] 21. 实现 Node Retire 的锁顺序与 single DB boundary，覆盖 current close/future
  cancellation、binding close、`account_inventory_poll_runs` abandonment、audit/receipt
  （≤2h）。
- [x] 22. 实现 Node Replace 的锁顺序与 single replacement boundary，覆盖 new identity、
  old/new lifecycle、lineage 和 no-inheritance（≤2h）。
- [x] 23. 实现 Retire/Replace replay、actor mismatch、intent mismatch、stale revision、
  duplicate identity 和 commit-unknown response recovery（≤2h）。
- [x] 24. 验证所有 lifecycle timestamps 使用 DB time，poll run transport evidence 不被覆盖并
  记录 abandonment transition time 原因（≤2h）。

## Monitoring and binding

- [x] 25. 更新所有 monitoring writers 先锁 Node 再按 `effective_from,activation_id`
  稳定锁 rows，增加 lock-order regression test（≤2h）。
- [x] 26. 实现 current interval boundary close 的合法 edge handling，禁止 retrospective
  UTC history rewrite（≤2h）。
- [x] 27. 实现 future monitoring cancellation 的 durable query/read model 与 counts（≤2h）。
- [x] 28. 实现 Node lifecycle binding close `node_retired/node_replaced`，验证不拿
  Gateway/Directory lock 的 inversion guard（≤2h）。
- [x] 29. 覆盖 binding-vs-Retire、binding-vs-Replace、monitor-writer-vs-Retire/Replace
  races 与 partial unique constraints（≤2h）。

## Inventory and Node-owned durable work

- [x] 30. 更新 Inventory scheduler eligibility：active Node、non-cancelled monitoring、
  capability、policy，禁止 retired poll（≤2h）。
- [x] 31. 更新 claim/preflight fence，处理 pending/retry_wait retired run 为 durable
  abandoned，确保 restart 不重开（≤2h）。
- [x] 32. 更新 outbound pre-dispatch fence 为 dispatch authorization 短事务：读取单一
...
- [x] 32a. 补 dispatch authorization 前置校验测试：lease 已过期但 fencing token 未变 →
...
- [x] 33. 更新 finalize/promotion fence：按 poll/run 所需锁后取得 Node `FOR UPDATE`，读取单一
  `database_now`，先判定 `node_retired|node_replaced`，再判定 active Node 是否存在
  `cancelled_at IS NULL AND database_now <@ active_range` 的 monitoring activation。active 但
  monitoring-ineligible 时 run 保持 finalized，run 与全部 pinned active Provider 使用
  `monitoring_ineligible`、Provider `promotion_applied=false`；冻结 precedence：Node lifecycle
  > monitoring_ineligible > policy_changed > provider-specific。证明 monitoring writer 与
  finalize 两种 Node-lock commit order、自然越过 `effective_to`、historical evidence 保留，且
  snapshot/current pointer/account lifecycle/availability/request-quality/provider-health current
  truth 均不更新（≤2h）。
- [x] 34. 验证 old identity 不会被隐式 retarget 到 replacement identity（≤2h）。
- [x] 35. 为 `account_inventory_poll_runs` 增加 `execution_reason IN
...
- [x] 36. 验证已有 transport evidence 的 poll run 不被伪造成 abandoned，而是走既有 finalize
  路径并由 promotion fence 跳过 current truth；runnable index/retry/reconciler 不会复活旧
  work（≤2h）。
- [x] 36a. 验证持有效 current-attempt authorization 的 `running` poll run 在所属 Node
...
- [x] 36b. 实现/验证 reconciler 对 lease 已过期的 `running` run 的处置顺序：先重新读取所属 Node
...
- [x] 37. 更新 OpenAPI Register/Edit/Retire/Replace exact method/path/body/status/error
  schemas，锁定 revision decimal string 和 Secret redaction（≤2h）。
- [x] 38. 更新 Node handlers 的 session/super_admin/CSRF/no-store/security-first error
  mapping 和 bounded receipt replay（≤2h）。
- [x] 39. 更新 Node list/detail query，读取 `asset_registry_generations.node_generation`
...
- [x] 40. 更新 `GetAssetCounts` 保持 legacy `nodes=total`，新增 `node_counts` active/
  retired/total（≤2h）。
- [x] 41. 运行 `make generate` 生成 Go/TypeScript clients，验证 generated files 可重现，
  不手工编辑 generated code（≤2h）。
- [x] 42. 补 read API pagination `generation` stale（`409 cursor_stale`）、filter
  mismatch、default active 和 historical detail integration tests（≤2h）。
- [x] 42a. 补跨 `effective_from`/`effective_to` boundary 的分页测试：验证已发 cursor chain
...
- [x] 43. 实现 Node Register/Edit 表单控件，仅调用本 change 冻结的 mutation 路径，
  `secret` 输入只提示 `secret_configured`、不回显引用（≤2h）。
- [x] 44. 实现 Node Retire 确认与 Replace 流程控件，覆盖 `expected_revision` 冲突时的刷新
  提示，不静默重试（≤2h）。
- [x] 45. 实现 retired Node 历史详情页，展示 predecessor/successor 导航，不提供任何使其
  复活为 current 的操作（≤2h）。
- [x] 46. 验证 UI 不显示 Health、Connection Test、Monitoring Enable/Disable 控件（仍属于
  `add-relay-node-management-operations`）（≤2h）。

## Audit, metrics, security and operational reads

- [x] 47. 扩展 `asset_node` audit category/action/check allowlist，验证成功 transition
  exactly one、replay zero second audit（≤2h）。
- [x] 48. 接入 `control_asset_mutation_total` Node bounded labels/result taxonomy，验证
  Node ID/secret/endpoint/actor 不出现在 labels（≤2h）。
- [x] 49. 更新 Inventory current、availability、request-quality reads 显式 active+
  monitoring eligibility fence，保留历史 evidence/rollup（≤2h）。
- [x] 50. 验证 authentication、CSRF、no-store、secret/credential/raw response negative
  cases 及不新增 arbitrary HTTP client（≤2h）。

## Race, crash and compatibility acceptance

- [x] 51. 补 concurrent Register same identity、concurrent Retire、concurrent Replace、
  Retire-vs-Replace、Edit-vs-Retire tests（≤2h）。
- [x] 52. 补 monitoring cancellation reaches effective_from、restart-after-cancellation、
  future expected-slot and GiST non-overlap tests（≤2h）。
- [x] 53. 补 Inventory claim/outbound/promotion-vs-Retire、old worker-after-Replace 和
  `account_inventory_poll_runs` restart tests（≤2h）。
- [x] 54. 补 crash-before-commit no-partial-state、commit-response-loss replay original
  result/status tests（≤2h）。
- [x] 55. 通过正式 wrapper 验证 pinned class1 rollback against floor2，证明 process/workers/
  HTTP 未启动且 DB truth 不变（≤2h）。
- [x] 56. 运行 signed manifest tamper/digest/key/DB failure fail-closed acceptance（≤2h）。

## Build, evidence and readiness

- [x] 57. 运行 Node-specific PostgreSQL 18 migration/ACL/constraint acceptance 并保存脱敏
  evidence（≤2h）。
- [x] 58. 运行 targeted integration tests、restart/replay/race selectors 并记录结果（≤2h）。

...
- 冻结 `account_key` 隐私边界：`account_key = normalized_provider + ':' + normalized_email` 仍是 Account Inventory 唯一 ownership identity，不建立第二套业务 identity；但新的 occurrence/evidence persistence 默认不得重复存储 plaintext account_key/完整 normalized email，须优先复用既有 masked/HMAC identity 或安全引用；UI/metrics/alerts/logs 永远不得输出 plaintext account_key；若实现阶段证明必须持久化 plaintext account_key，须先停止并单独 security review。

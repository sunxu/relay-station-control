...
- [x] 2.1 实施最小forward migration、固定审计入口及有效lease限定的runtime目标只读函数，保持既有register不变。
- [x] 2.2 复用deploy/asset-registry惯例添加SQL registrar模板（不新增CLI/API/UI），以受保护输入传递reference，明确actor、NULL前置条件和幂等输出。
- [x] 2.3 真实PG验证首次补全、非NULL冲突、no-op、非法输入、全部历史拒绝与audit原子回滚。
- [x] 2.4 真实PG验证目标读取有效/过期/错误fencing与reference直接SELECT拒绝，以及补填runtime/PUBLIC拒绝、并发补全/首次调度/Binding、锁超时及Up/Down兼容历史。

## 3. Directory runtime integration

- [x] 3.1 实现enabled/映射两项配置校验与独立SecretResolver，disabled零副作用。
- [x] 3.2 接线ReconcileTick→WorkOnce及受限runtime pool、取消和等待；复用RunGatewayOnce内ScheduleCurrent，不额外调用ScheduleTick，用合成工作项验证慢/失败前项后续仍处理，真实单Gateway PG验证当前槽去重。
- [x] 3.3 接线既有metrics并验证状态、固定标签与日志脱敏。
- [x] 3.4 真实PG及进程测试验证180s/540s预算、restart、两个实例、lease loss、unknown commit、shutdown。
- [x] 3.5 落实独立5s attempt及最多5s失败收尾context，验证慢header/body/finalize、取消、失lease和unknown commit，不延长成功或lease预算。
- [x] 3.6 在共同ScheduleCurrent行锁内用reader_secret_configured实现NULL no-work，真实单Gateway PG验证补填竞争两种顺序及NULL/凭据失败，合成遍历验证A未配置/B成功与A凭据失败/B成功；保留历史run恢复。

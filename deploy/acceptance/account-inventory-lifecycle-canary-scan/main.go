...
var (
	errInvalidConfiguration = errors.New("lifecycle canary configuration invalid")
	errUnsafeArtifact       = errors.New("lifecycle canary artifact unsafe")
	errSensitiveCanary      = errors.New("lifecycle canary found")
)
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "account_inventory_lifecycle_canary_scan=failed reason=fixed_security_check")
		os.Exit(1)
	}
	fmt.Println("account_inventory_lifecycle_canary_scan=success")
}
...
func run() error {
	...
	return scanDirectory(os.Getenv("CONTROL_LIFECYCLE_CANARY_SCAN_DIR"), canaries)
}

...
wait_for_http() {
  local layer="$1" url="$2" attempts="$3" attempt=0
  ...
  echo "${layer}_READINESS_FAILURE target=$url attempts=$attempt" >&2
  return 1
}
...
wait_for_node_ready() {
  ...
  while (( attempt < attempts )); do
    ...
    if [[ "$state" == "exited" || "$state" == "dead" ]]; then
      echo "${layer}_READINESS_FAILURE target=$node_container state=$state" >&2
      compose logs --no-color node >&2 || true
      return 1
    fi
    sleep 1
  done
  echo "${layer}_READINESS_FAILURE target=$node_container attempts=$attempt state=${state:-unknown} health=${health:-unknown}" >&2
  compose logs --no-color node >&2 || true
  return 1
}

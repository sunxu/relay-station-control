...
cleanup() {
  ...
  case "$runtime_directory" in
    /tmp/relay-control-history-run.*|/private/tmp/relay-control-history-run.*)
...
require_test() {
  ...
  if ! env -u HTTP_PROXY -u HTTPS_PROXY -u ALL_PROXY -u http_proxy -u https_proxy -u all_proxy \
    GOCACHE="$runtime_directory/go-build" go test "$package" -list "^${exact_name}$" >"$listing" 2>&1; then
...
run_static() {
  ...
  require_test ./cmd/control TestAccountInventoryHistoryRuntimeShutdownTimeoutLogIsFixed
...
cd "$repository_root"
runtime_directory="$(mktemp -d /tmp/relay-control-history-run.XXXXXX)"
trap cleanup EXIT

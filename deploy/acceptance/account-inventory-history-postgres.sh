...
main() {
  ...
  strict_cleanup
  echo 'account_inventory_history_postgres=success server_major=18 migration=9 up_down_up=covered core_sha256=covered canonical_golden=covered compaction_main_path=covered summarize_atomicity=covered summarize_immutability=covered final_rollup=covered finalize_atomicity=covered final_immutability=covered metrics_completed_only=covered zero_provider=covered planner_catalog_utc_inclusive_72h=covered planner_eligibility_matrix=covered finalize_catalog_9500=covered utc_dst_72h_expression=covered slot_provider_matrix=covered incomplete_segment_gates=covered coverage_expression_9499_finalize_9474_9500_10000=covered last_segment_concurrency=covered policy_boundary=covered metrics_backlog_drain=covered retention_batches=covered retention_planner_lock=covered planner_limit_progress=covered retired_day_poll_lock=covered legacy_retention_bootstrap=covered zero_poll_lineage_bootstrap=covered retired_day_no_resurrection=covered current_query_after_retention=covered lease_expiry=covered capacity_1_10_50_total_accounts=1000 audit_gate=covered runtime_table_dml=denied runtime_functions=allowlisted cleanup_containers=0 cleanup_volumes=0 cleanup_networks=0'
}

...
run_api_probe() {
  if ! env -u HTTP_PROXY -u HTTPS_PROXY -u ALL_PROXY -u http_proxy -u https_proxy -u all_proxy \
    GOCACHE="$runtime_directory/go-build" go test ./internal/api \
      -run '^TestAccountInventoryHTTPRepositoryRetentionNullSourceAndInconsistentCurrentState$' \
      -count=1 >"$runtime_directory/current-http-query.log" 2>&1; then
    fixed_failure 'postgres_current_http_query_failed'
  fi
}
...
main() {
  ...
  require_store_test TestAccountInventoryHistoryCompletionCountMismatchFailsClosedAndRetainsPolls
  require_store_test TestAccountInventoryHistoryCompactionClaimRenewReclaimFencing
  require_store_test TestAccountInventoryHistoryExpiredLeaseRequiresReconcileAcrossCompactionPhases
  ...
  require_store_test TestAccountInventoryHistoryPollRetentionChildFailuresRollbackAndResume
  require_store_test TestAccountInventoryHistoryPollRetentionRejectsIneligibleCandidates
  require_store_test TestAccountInventoryHistoryRetentionEligibilityBoundaries
  ...
  require_store_test TestAccountInventoryRowsFailClosed
  require_api_test TestAccountInventoryHTTPRepositoryRetentionNullSourceAndInconsistentCurrentState

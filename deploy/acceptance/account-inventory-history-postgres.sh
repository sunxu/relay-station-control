...
require_test() {
  ...
  if ! env -u HTTP_PROXY -u HTTPS_PROXY -u ALL_PROXY -u http_proxy -u https_proxy -u all_proxy \
    go test ./deploy/acceptance/account-inventory-history-postgres \
...
run_store_race_probe() {
  if ! env -u HTTP_PROXY -u HTTPS_PROXY -u ALL_PROXY -u http_proxy -u https_proxy -u all_proxy \
    go test -race ./internal/store \
      -run '^TestAccountInventoryHistoryConcurrentRetentionQueryPromotionAndScope$' \
      -count=1 >"$runtime_directory/store-concurrency-race.log" 2>&1; then
    fixed_failure 'postgres_retention_query_promotion_scope_race_failed'
  fi
}
...
main() {
  ...
  assert_version 52 migration-version-up.log
  if ! compose exec -T postgres psql --username relay_control_migrator --dbname relay_station_control \
  ...
  require_store_test TestAccountInventoryHistoryConcurrentRetentionQueryPromotionAndScope
  ...
  require_store_test TestAccountInventoryHistoryAuditExactAllowlistAndRetentionBoundary
  require_store_test TestAccountInventoryHistorySensitiveCanaryDatabaseSinks
  require_store_test TestAccountInventoryHistoryCrashRecoveryMatrix
  require_store_test TestAccountInventoryHistorySecurityBoundaryMatrix
  require_store_test TestAccountInventoryHistoryAggregateSchemaConstraintAndProtectionGateMatrix
  ...
  assert_version 52 migration-version-repeat.log

...
func TestNodeLifecycleCommandsReplayAndLineagePG18(t *testing.T) {
	...
	defer pool.Close()
	repository, err := assetstore.NewNodeLifecycleRepositoryWithSealer(pool, []byte("01234567890123456789012345678901"), newAvailableTestSealer())
	if err != nil {
		t.Fatal(err)
	}
...
func TestNodeInventoryClaimTerminalizesRetiredPendingRunPG18(t *testing.T) {
	...
	if _, err := owner.Exec(ctx, `INSERT INTO relay_node_assets(instance_id,display_name,node_type,driver_contract_version,management_endpoint,lifecycle_status,revision) VALUES($1,'Claim Node','cliproxyapi','v1','http://claim-node.example','active',1)`, nodeID); err != nil {
		t.Fatal(err)
	}

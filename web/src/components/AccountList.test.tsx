...
describe("AccountList", () => {
  ...
  it("uses locale-aware defaults and preserves an explicit empty override", () => {
    render(<FrontendFoundationProvider initialLocale="en"><AccountList rows={[]} /></FrontendFoundationProvider>);
    expect(screen.getByText("No account matches the current filters")).toBeInTheDocument();
    render(<FrontendFoundationProvider initialLocale="en"><AccountList rows={[]} emptyDescription="Custom empty state" /></FrontendFoundationProvider>);

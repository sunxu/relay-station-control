...
export function AccountQualityIncidentsSection({ api, instanceId, providers, providerError, onSelectAccount, onUnauthorized }: {
  ...
  const columns: ColumnsType<AccountQualityIncidentItem> = [
    { title: copy.incidentAccount, dataIndex: "account_key", render: (value: string) => <Button data-testid={`account-incident-${encodeURIComponent(value)}`} type="link" size="small" onClick={() => onSelectAccount(value)}>{value}</Button> },
    ...
    { title: copy.hits, dataIndex: "hit_count", render: (value: number) => formatNumber(value, locale) },
  ...
  return <Card title={copy.incidents} role="region" aria-label={copy.incidents}>
    <Flex gap={8} wrap>
      <Select data-testid="account-incidents-provider-filter" allowClear aria-label={copy.incidentProvider} placeholder={copy.allProviders} value={provider} onChange={(v) => { setProvider(v); setCursor(undefined); }} options={providers.map((row) => ({ value: row.provider, label: row.provider }))} disabled={providerError} />
      <Select data-testid="account-incidents-reason-filter" allowClear aria-label={copy.incidentReasonFilter} placeholder={copy.allReasons} value={failureClass} onChange={(v) => { setFailureClass(v); setCursor(undefined); }} options={failureOptions} />
      <Button data-testid="account-incidents-refresh" onClick={() => void query.refetch()} loading={query.isFetching}>{copy.refreshIncidents}</Button>
    </Flex>
    ...
    {query.data && !query.error && <>
      ...
      <Flex justify="end" gap={8} style={{ marginTop: 8 }}><Button data-testid="account-incidents-first" disabled={!cursor} onClick={() => setCursor(undefined)}>{copy.incidentsFirst}</Button><Button data-testid="account-incidents-next" disabled={!query.data.next_cursor || query.isFetching} onClick={() => setCursor(query.data.next_cursor ?? undefined)}>{copy.incidentsNext}</Button></Flex>
    </>}
  </Card>;
}

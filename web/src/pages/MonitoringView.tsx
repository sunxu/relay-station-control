import { Alert, Button, Card, Empty, Flex, Input, Select, Spin, Table, Tag, Typography } from "antd";
...
function ReadError({ retry, message, retryLabel, testId }: { retry: () => void; message: string; retryLabel: string; testId?: string }) {
  return <Alert type="error" title={message} action={<Button data-testid={testId} onClick={retry}>{retryLabel}</Button>} />;
}
function Evidence({ api, occurrenceId, onUnauthorized, copy }: { api: TopologyApi; occurrenceId: string; onUnauthorized: () => void; copy: TranslationResource["monitoring"] }) {
  ...
  return <Flex vertical gap={8}>
    ...
    <Flex gap={8} justify="end">
      <Button data-testid="monitoring-evidence-first" disabled={!cursor} onClick={() => setCursor(undefined)}>{copy.evidenceFirstPage}</Button>
      <Button data-testid="monitoring-evidence-next" disabled={!query.data.next_cursor || query.isFetching} onClick={() => setCursor(query.data.next_cursor ?? undefined)}>{copy.evidenceNextPage}</Button>
    </Flex>
  </Flex>;
}

export function MonitoringView({ api, assetApi, inventoryApi, initialInstanceId, csrfToken = "", onUnauthorized, onNavigate }: {
  api: TopologyApi; assetApi: AssetApi; inventoryApi?: AccountInventoryApi; initialInstanceId?: string; csrfToken?: string; onUnauthorized: () => void; onNavigate?: (route: "nodes" | "accounts" | "problems" | "assets") => void;
}) {
  const cache = useQueryClient();
  const locale = useOptionalAppLocale()?.locale ?? "zh-CN";
  const copy = resources[locale].translation.monitoring;
  ...
  const select = (id: string) => {
    ...
    setHistoryCursor(undefined);
    setCurrentCursor(undefined);
    const basePath = window.location.pathname.replace(/\/$/, "") === "/topology" ? "/topology" : "/monitoring";
    window.history.pushState(null, "", `${basePath}?instance_id=${encodeURIComponent(id)}`);
  };
  ...
  return <Flex vertical gap={16} data-testid="monitoring-view" className="monitoring-view" style={{ minWidth: 0 }}>
    ...
    {inventoryApi && <AccountInventoryCapacity api={inventoryApi} csrfToken={csrfToken} onUnauthorized={expireSession} surface="monitoring" />}
    <Card title={copy.navigationTitle} data-testid="monitoring-navigation">
      <Flex gap={8} wrap>
        <Button data-testid="monitoring-open-nodes" onClick={() => onNavigate?.("nodes")}>{copy.openNodes}</Button>
        <Button data-testid="monitoring-open-accounts" onClick={() => onNavigate?.("accounts")}>{copy.openAccounts}</Button>
        <Button data-testid="monitoring-open-problems" onClick={() => onNavigate?.("problems")}>{copy.openProblems}</Button>
        <Button data-testid="monitoring-open-assets" onClick={() => onNavigate?.("assets")}>{copy.openAssets}</Button>
      </Flex>
    </Card>
    ...
    {instanceId && <>
      ...
      <Card className="topology-card" data-testid="monitoring-history-ownership" title={copy.ownershipHistory}>
        ...
        {history.data && !history.error && <>
          ...
          <Flex justify="end" gap={8}><Button data-testid="monitoring-history-first" disabled={!historyCursor} onClick={() => setHistoryCursor(undefined)}>{copy.firstPage}</Button><Button data-testid="monitoring-history-next" disabled={!history.data.next_cursor || history.isFetching} onClick={() => setHistoryCursor(history.data?.next_cursor ?? undefined)}>{copy.nextPage}</Button></Flex>
        </>}
      </Card>
    </>}
  </Flex>;

  ...
  return (
    <Flex vertical gap={16} data-testid="problems-view">
      ...
      <Card title={copy.title} data-testid="problems-card">
        ...
        {!list.error && list.data && list.data.items.length > 0 && <Table<ProblemAccountItem> rowKey={(row) => `${row.instance_id}:${row.account_key}`} onRow={(row): HTMLAttributes<HTMLTableRowElement> => ({ "data-testid": `problem-row-${row.instance_id}-${encodeURIComponent(row.account_key)}`, "data-instance-id": row.instance_id, "data-account-key": row.account_key } as unknown as HTMLAttributes<HTMLTableRowElement>)} size="small" scroll={{ x: 1500 }} pagination={false} dataSource={list.data.items} columns={columns} />}

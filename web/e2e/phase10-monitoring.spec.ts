...
async function install(page: Page) {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (url.pathname === `/api/topology/nodes/${nodeA}/duplicate-history` || url.pathname === `/api/topology/nodes/${nodeB}/duplicate-history`) return route.fulfill({ json: { involvement: "historical", instance_id: url.pathname.includes(nodeB) ? nodeB : nodeA, observed_at: "2026-09-07T00:00:00Z", items: [occurrence("history-1", "RESOLVED")], next_cursor: "history-2" } });
...
async function assertInteractiveDiagnostics(page: Page) {
  ...
  await page.getByTestId(`monitoring-node-option-${nodeB}`).click();
  await expect(page.getByTestId("monitoring-view")).toContainText(nodeB);
  await page.getByTestId("monitoring-node-first").click();
  await page.getByTestId("monitoring-node-selector").click();
  await page.getByTestId(`monitoring-node-option-${nodeA}`).click();
  await expect(page.getByTestId("monitoring-view")).toContainText(nodeA);

...
test("Problems is a lazy read-only view with one query per page", async ({ page }, testInfo) => {
  ...
  const nodeARow = page.locator(`[data-testid="problem-row"][data-instance-id="${nodeA}"]`);
  const nodeBRow = page.locator(`[data-testid="problem-row"][data-instance-id="${nodeB}"]`);
  await expect(nodeARow).toBeVisible();
  await expect(nodeBRow).toBeVisible();
  await expect(nodeARow.getByText("TOKEN_INVALID", { exact: true })).toBeVisible();
  await expect(nodeARow.getByText("FORBIDDEN", { exact: true })).toBeVisible();
  await expect(nodeBRow.getByText("ACCOUNT_BLOCKED", { exact: true })).toBeVisible();
